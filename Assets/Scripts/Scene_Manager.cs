...
public class Scene_Manager : MonoBehaviour
{
    ...
    public void checkLevelFile()
    {
        ...
        if (!File.Exists(ruta1)) File.Copy(Path.Combine(Application.streamingAssetsPath, "nivel_Info.txt"), ruta1);
        if (!File.Exists(ruta2)) File.Copy(Path.Combine(Application.streamingAssetsPath, "Levels.txt"), ruta2);
        if (!File.Exists(ruta3)) File.Copy(Path.Combine(Application.streamingAssetsPath, "ClienteInfo.txt"), ruta3);
        if (!File.Exists(ruta4)) File.Copy(Path.Combine(Application.streamingAssetsPath, "Cards_Info.txt"), ruta4);
        if (!File.Exists(ruta5)) File.Copy(Path.Combine(Application.streamingAssetsPath, "IntroductoryLevels.txt"), ruta5);
    }
    #endregion
}
